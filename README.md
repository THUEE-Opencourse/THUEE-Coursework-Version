# THU-EE-Coursework-Version

## 核心课
按拼音字母顺序排列
- [程序设计基础(1)](CoreCourses/程序设计基础(1).md)
- [程序设计基础(2)](CoreCourses/程序设计基础(2).md)
- [程序设计基础(3)](CoreCourses/程序设计基础(3).md)
- [概率论与随机过程(1)-英](CoreCourses/概率论与随机过程(1)-英.md)
- [媒体与认知-英](CoreCourses/媒体与认知-英.md)
- [数字逻辑与处理器基础实验](CoreCourses/数字逻辑与处理器基础实验.md)
- [通信与网络](CoreCourses/通信与网络.md)

## 其他必修课
- [MATLAB高级编程与工程应用](OtherCompulsoryCourses/MATLAB高级编程与工程应用.md)
- [物理电子学实验](OtherCompulsoryCourses/物理电子学实验.md)
- [物理实验A(1)](OtherCompulsoryCourses/物理实验A(1).md)

## 限选课
- [编码引论](LimitedElectiveCourses/编码引论.md)
